...
MatrixD* JacMatrix(void* implementor, VectorD* param, VectorD* wls,
    ...
    for (i = 0; i < half_size; i++) {
        ...
        Film_SetConfig(film, 0, wl, 1, 0);
        Film_Calculate(film);
        // I think rewrapping is basically pointing grad to jacobi_ptr + i * ld
...
void insert_layer(Film* film, VectorD* wls, VectorD* target_spec,
    ...
    for (layer = 0; layer < layer_number; layer++) {
        ...
        for (pos = 0; pos < pos_max; pos += pos_step) {
            ...
            grad = 0;
            for (wl_i = 0; wl_i < wl_i_half; wl_i++) {
                // te
            ...
            for (wl_i = wl_i_half; wl_i < wl_i_max; wl_i++) {
                // te
                wl = VectorD_GetElement(wls, wl_i);
                Film_SetConfig(film, 60, wl, 1, 0);
                Film_Calculate(film);
    ...
    Film_InsertLayer(film, best_insert_layer_num + 2, insert_material, 0.0001,
                     1);
...
DWORD WINAPI run(LPVOID args_ptr) {
    ...
    // need duplicated wls!
    sample_pts_num = File_Size("wl_0_and_60deg_R.txt");
    // wavelengths of all sampling points
    wls = VectorD_New(sample_pts_num);
    VectorD_ReadFromFile(wls, "wl_0_and_60deg_R.txt");
    sample_pts_num = File_Size("spec_0_and_60deg_R.txt");
    target_spec = VectorD_New(sample_pts_num);
    VectorD_ReadFromFile(target_spec, "spec_0_and_60deg_R.txt");
