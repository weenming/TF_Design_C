...
void insert_layer(Film* film, VectorD* wls, VectorD* target_spec,
    ...
    printf("insert gradient: %f\n", best_grad);
    if (best_grad > -1e-6 || best_insert_pos == 0) {
        printf("cannot find insertion position! \nappending\n");
        // when last layer is SiO2, add thickness
        // when the last layer is TiO2, append a new layer of SiO2

        if (layer_number % 2 == 1) {
            // add 100 nm to the last layer
            Film_GetOptParam(film, vector_d);
            Film_SetOptParam(
                film,
                VectorD_SetElement(
                    vector_d, layer_number - 1,
                    VectorD_GetElement(vector_d, layer_number - 1) + 0.1));
            Film_Build(film);
            Film_Calculate(film);
        } else {
            Film_InsertLayer(film, layer_number + 1, even_material, 0.1, 1);
            Film_Build(film);
            Film_Calculate(film);
        }
...
void substitute_film(Film* film, double allowed_thickness) {
    ...
    while (i != Film_GetOptSize(film)) {
        ...
        if (VectorD_GetElement(d, i) < allowed_thickness && i < layer_num - 2) {
            VectorD_SetElement(
                d, i - 1,
...
DWORD WINAPI run(LPVOID args_ptr) {
    ...
    int run_count = (*(args*)args_ptr).run_count, i;
    MaterialN* air;
    MaterialS *sio2, *tio2;
    ...
    // initialize
    double allowed_thickness = (*(args*)args_ptr).allowed_thickness;
    IMaterial* material_arr[2] = {SiO2, TiO2};
    char load_d_fpath[200];
    sprintf(load_d_fpath,
            "./../result/LTM_1_delete_on_50_iters_denser/run_%d/final",
            run_count);
    char fpath[200];
    sprintf(
        fpath,
        "./../result/LTM_1_delete_on_50_iters_denser/run_%d/subs_allow_%.2f",
        run_count, allowed_thickness * 1000);  // in nm

    // Load final res from file
    int d_len = File_Size(load_d_fpath);
    VectorD* init_d = VectorD_New(d_len);
    VectorD_ReadFromFile(init_d, load_d_fpath);
    for (i = 0; i < d_len; i++) {
        Film_AddLayer(film, material_arr[i % 2], VectorD_GetElement(init_d, i),
                      1);
    }
    ...
    // 1st material argument: odd layer; 2nd material argument: even layer
    // (the layer at the top)
    int last_layer_number, this_layer_number;
    do {  // substitute
        last_layer_number = Film_GetOptSize(film);
        substitute_film(film, allowed_thickness);
        d = VectorD_New(Film_GetOptSize(film));
        Film_GetOptParam(film, d);
        ...
        substitute_film(film, allowed_thickness);
        this_layer_number = Film_GetOptSize(film);
        // Need to recursively substitute layers
    } while (this_layer_number != last_layer_number);
    ...
    // The bug should be caused by this problem: d_final is not given the
    // value after insertions of d, and therefore has wrong shape for the
    // film when calculating the spectrum. The peculiar part is that the
...
int main() {
    ...
    HANDLE thread[J_SIZE];
    void* p;
    VectorD* ratios = VectorD_New(RATIO_SIZE);

    VectorD_Linspace(ratios, 3, 9);
    VectorD_MulNumD(ratios, 1.888, ratios);

    // 必须给不同参数指定不同的地址，传入不同线程的函数
    // 初始化参数
    args p_arr[RATIO_SIZE * J_SIZE];
    int j;
    for (j = 1; j < J_SIZE + 1; j++) {
        for (i = 0; i < RATIO_SIZE; i++) {
            p_arr[i * J_SIZE + j - 1].thickness = VectorD_GetElement(ratios, i);
            p_arr[i * J_SIZE + j - 1].run_count = i;
            // 1nm
            p_arr[i * J_SIZE + j - 1].allowed_thickness = 1e-4 * j;
        }
    }

    for (i = 194; i < RATIO_SIZE; i++) {
        for (j = 0; j < J_SIZE; j++) {
            p = &p_arr[i * J_SIZE + j];
            thread[j] = CreateThread(NULL, 0, run, p, 0, NULL);
        }
        for (j = 0; j < J_SIZE; j++) {
