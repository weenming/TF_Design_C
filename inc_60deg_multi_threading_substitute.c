...
void delete_film(Film* film) {
    ...
    while (i != Film_GetOptSize(film)) {
        ...
        if (VectorD_GetElement(d, i) == 0 && i != layer_num - 1) {
            VectorD_SetElement(
                d, i - 1,
...
void substitute_film(Film* film) {
    int layer_num = Film_GetOptSize(film);
    int i = 1;
    VectorD* d;
    while (i != Film_GetOptSize(film)) {
        ...
        Film_GetOptParam(film, d);
        // VectorD_Show(d);
        // last layer not substituted
        if (VectorD_GetElement(d, i) < 5e-3 && i < layer_num - 2) {
            if (i % 2 == 0) {
                // i is even: i-th layer is SiO2
                // Substitute with same OT TiO2
                VectorD_SetElement(d, i - 1,
                                   VectorD_GetElement(d, i - 1) +
                                       VectorD_GetElement(d, i + 1) +
                                       0.6 * VectorD_GetElement(d, i));
            } else {
                // i is odd: i-th layer is TiO2
                // Substitute with same OT SiO2
                VectorD_SetElement(d, i - 1,
                                   VectorD_GetElement(d, i - 1) +
                                       VectorD_GetElement(d, i + 1) +
                                       1.666 * VectorD_GetElement(d, i));
            }
...
void test_deletion() {
    ...
    Film_AddLayer(film, SiO2, 0.001, 1);
    Film_AddLayer(film, SiO2, 0.00499, 1);
    Film_AddLayer(film, SiO2, 0.005, 1);
...
void test_substitution() {
    ...
    film = Film_New(0, 0.5, 1, 1);
    Film_AddLayer(film, Air, 0, 0);
    Film_AddLayer(film, SiO2, 0.0001, 1);
    Film_AddLayer(film, TiO2, 0.0499, 1);
    Film_AddLayer(film, SiO2, 0.05, 1);
    Film_AddLayer(film, TiO2, 0.0001, 1);
    Film_AddLayer(film, SiO2, 0.001, 1);
    Film_AddLayer(film, TiO2, 0.0499, 1);
    Film_AddLayer(film, SiO2, 0.05, 1);
    Film_AddLayer(film, TiO2, 0.0001, 1);
